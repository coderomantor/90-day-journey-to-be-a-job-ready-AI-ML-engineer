---
noteId: "3ac84a505e5211f19b67039edb4b38c8"
tags: []

---

# Week 01 Review

## Focus

This week is focused on strengthening Python and data handling foundations before moving deeper into machine learning.

## What I Learned

- How NumPy arrays help with numerical data.
- How to calculate totals, averages, highest values, and lowest values using vectorized operations.
- How to document a small project clearly for GitHub visitors.

## What I Built

- Day 01 Student Marks Analyzer using NumPy.

## What Was Difficult

- Thinking in rows and columns instead of only simple Python lists.
- Understanding when to use `axis=0` and `axis=1` in NumPy.

## What I Should Improve

- Practice more NumPy examples with real datasets.
- Add screenshots of project output.
- Keep each daily README clear and short.

## Notes For Next Week

- Continue with NumPy practice.
- Start Pandas basics.
- Build one small data analysis project before moving into machine learning models.
